...
describe(List) do
  ...
  describe(List) do
    it("tells which tasks are in it") do
      list = List.create({:name => "Farm chores"})
      task1 = Task.create({:description => "Clean out the mower", :list_id => list.id()})
      task2 = Task.create({:description => "Clean out the mower", :list_id => list.id()})
